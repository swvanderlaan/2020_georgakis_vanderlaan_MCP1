...
Load those packages.
```{r Setting: loading_packages, message=FALSE, warning=FALSE, include=}
...
In a separate notebook we provide the results from the scRNAseq analysis.
...
In this model we correct for _Age_, _Gender_, and _year of surgery_ (`COVARIATES_M1`).

Here we use the inverse-rank normalized data - visually this is more normally distributed.

Analysis of the plaque vulnerability indez as a function of plaque cytokine levels.
```{r CrossSec: Plaque_Vulnerability_Index - ordinal regression MODEL1 RANK, cytokines, include=TRUE, paged.print=TRUE}
...
```{r CrossSec: Plaque_Vulnerability_Index - ordinal regression MODEL2 RANK, cytokines, include=TRUE, paged.print=TRUE}
...
```{r CrossSec: Plaque_Vulnerability_Index - ordinal regression MODEL3 RANK, cytokines, include=TRUE, paged.print=TRUE}
